<div class="form-group {{ $class or '' }} form-group-default {{ isset($required) && $required ? 'required' : '' }}">
    ...
    <input id="{{ $id or '' }}" name="{{ $name or '' }}" value="{{ $value or '' }}" type="number" min="{{ $min or '-99999999' }}" max="{{ $max or '99999999' }}" step="{{ $step or '0.01' }}" class="form-control" placeholder="{{ $placeholder or '' }}" {{ isset($required) && $required ? 'required' : '' }} />
</div>
@if(isset($note))
